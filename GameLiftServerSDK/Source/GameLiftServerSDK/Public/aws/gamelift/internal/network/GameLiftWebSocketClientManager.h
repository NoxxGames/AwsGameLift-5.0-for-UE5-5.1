...
namespace Internal {
    ...
    /**
     * Class Managing a single WebSocketClientWrapper, enabling connection and communication with GameLift
     */
    class GameLiftWebSocketClientManager {
    public:
        GameLiftWebSocketClientManager(std::shared_ptr <IWebSocketClientWrapper> webSocketClientWrapper)
        ...
        Aws::GameLift::GenericOutcome Connect(std::string websocketUrl, const std::string& authToken,
                const std::string& processId, const std::string& hostId, const std::string& fleetId);
