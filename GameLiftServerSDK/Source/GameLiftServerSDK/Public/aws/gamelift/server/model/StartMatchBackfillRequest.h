...
/**
    ...
    *
    * Note: Each StartMatchBackfillRequest takes a little less than 1mb of memory. The default windows stack size is 1mb,
    * so creating one StartMatchBackfillRequest variable on the stack frame gets you 99% of the way to a stack overflow.
    * On MacOS and Linux the stack size is up to 8mb so the issue is seen less often but is still possible. Avoid creating
    * local variables of this type.
    */
...
public:
    ...
    */
    ...
    inline StartMatchBackfillRequest& WithPlayer(Player player) { return AddPlayer(player);}
